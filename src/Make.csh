...
if ($1 == "Makefile.lib") then
  ...
  set list = `grep -l FFT_CLASS fft_*.h`
  if (-e style_fft.tmp) then
    rm style_fft.tmp
  endif
  foreach file ($list)
    set qfile = \"$file\"
    echo "#include $qfile" >>! style_fft.tmp
  end
  if (! -e style_fft.tmp) then
     rm style_fft.h
     touch style_fft.h
  else if (! -e style_fft.h) then
     mv style_fft.tmp style_fft.h
     rm Obj_*/input.d
  else if (`diff style_fft.h style_fft.tmp` != "") then
     mv style_fft.tmp style_fft.h
     rm Obj_*/input.d
  else
     rm style_fft.tmp
